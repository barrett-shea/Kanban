...
export default function createBoard(id) {
  firestore.collection("users").doc(id).set({
    createdAt: database.getCurrentTimestamp() 
  })
  firestore.collection("users").doc(id).collection('tasks')
  firestore.collection("users").doc(id).collection('columns').doc('column1').set({
    id = 'column1',
    taskIds = [],
    title = 'To Do',
    userId = id
